...
def give_best_top(query):
    query+=f"Make sure all The features like colour should belong to list {colours},gender should be male, female or unisex, type of shirt should belong to list {types}, and season should belong to list {season} and specify accordingly please in this format 'gender colour type season', give only these values and give only one option"
...
# print(give_best_features(f"{prompt} specify accordingly please \ncolour: \n type: \ngender: and give only one option"))
...
def find_best_match_top(item):
    ...
    # Create the model, looking for the single closest neighbor
    nn = NearestNeighbors(n_neighbors=1, algorithm='ball_tree')
    nn.fit(encoded_labels)

    # Find the closest vector
    distances, indices = nn.kneighbors([encoded_item])
...
def find_best_match_bottom(item):
    ...
    with open('C:\\Users\\krish\\OneDrive\\Desktop\\SE Hackathon\\footwear_labels.pkl', 'rb') as fp:
...
def find_best_match_accessory(item):
    ...
    with open('C:\\Users\\krish\\OneDrive\\Desktop\\SE Hackathon\\accessory_labels.pkl', 'rb') as fp:
...
def give_best_bottom(query)      :
    ...
    query+=f"Make sure all The features like colour should belong to list {colours},gender should be male, female or unisex, type of shirt should belong to list {types}, and season should belong to list {season} and length should be either full or short, specify accordingly please in this format 'gender colour type season length', give only these values and give only one option"
...
print(recommend_accessory("Male","black","hat","all"))
