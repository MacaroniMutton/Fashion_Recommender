...
import pickle
from .models import Footwear, Accessories, Top, Bottom, Purchase_History_Accessories, Purchase_History_Bottom, Purchase_History_Footwear, Purchase_History_Top
...
def detail(request, men_women, title, id):
    ...
    if request.method=="POST":
        if table=="Top":
            Purchase_History_Top.objects.create(user=request.user, top=product)
        elif table=="Bottom":
            Purchase_History_Bottom.objects.create(user=request.user, bottom=product)
        elif table=="Footwear":
            Purchase_History_Footwear.objects.create(user=request.user, footwear=product)
        elif table=="Accessories":
            Purchase_History_Accessories.objects.create(user=request.user, acc=product)
...
def history(request):
    top = Purchase_History_Top.objects.last()
    bottom = Purchase_History_Bottom.objects.last()
    footwear = Purchase_History_Footwear.objects.last()
    acc = Purchase_History_Accessories.objects.last()
    ...
    if top:
        li = recommend_top(top.top.gender.lower(), top.top.color.lower(), top.top.type.lower(), top.top.season.lower())
        sim_tops = [x[0] for x in li]
        sim_top_urls = [f"top_images/{sim_top}" for sim_top in sim_tops]
        print(sim_top_urls)
        sim_1 = Top.objects.filter(image = sim_top_urls[0])[0]
        sim_2 = Top.objects.filter(image = sim_top_urls[1])[0]
        sim_3 = Top.objects.filter(image = sim_top_urls[2])[0]
        similars_1 = [sim_1, sim_2, sim_3]
    if bottom:
        li = recommend_bottom(bottom.bottom.gender.lower(), bottom.bottom.color.lower(), bottom.bottom.type.lower(), bottom.bottom.season.lower(), bottom.bottom.length.lower())
        sim_tops = [x[0] for x in li]
        sim_top_urls = [f"bottom_images/{sim_top}" for sim_top in sim_tops]
        print(sim_top_urls)
        sim_1 = Bottom.objects.filter(image = sim_top_urls[0])[0]
        sim_2 = Bottom.objects.filter(image = sim_top_urls[1])[0]
        sim_3 = Bottom.objects.filter(image = sim_top_urls[2])[0]
        similars_2 = [sim_1, sim_2, sim_3]
    if footwear:
        li = recommend_footwear(footwear.footwear.gender.lower(), footwear.footwear.color.lower(), footwear.footwear.type.lower(), footwear.footwear.season.lower())
        sim_tops = [x[0] for x in li]
        sim_top_urls = [f"footwear_images/{sim_top}" for sim_top in sim_tops]
        print(sim_top_urls)
        sim_1 = Footwear.objects.filter(image = sim_top_urls[0])[0]
        sim_2 = Footwear.objects.filter(image = sim_top_urls[1])[0]
        sim_3 = Footwear.objects.filter(image = sim_top_urls[2])[0]
        similars_3 = [sim_1, sim_2, sim_3]
    if acc:
        li = recommend_accessory(acc.acc.gender.lower(), acc.acc.color.lower(), acc.acc.type.lower(), acc.acc.season.lower())
        sim_tops = [x[0] for x in li]   
        sim_top_urls = [f"acc_images/{sim_top}" for sim_top in sim_tops]
        print(sim_top_urls)
        sim_1 = Accessories.objects.filter(image = sim_top_urls[0])[0]
        sim_2 = Accessories.objects.filter(image = sim_top_urls[1])[0]
        sim_3 = Accessories.objects.filter(image = sim_top_urls[2])[0]
        similars_4 = [sim_1, sim_2, sim_3]

    return render(request, 'myapp/history.html', {"similars_1": similars_1, "similars_2": similars_2, "similars_3": similars_3, "similars_4": similars_4, "top_hist": Purchase_History_Top.objects.all(), "bottom_hist": Purchase_History_Bottom.objects.all(), "footwear_hist": footwear, "acc_hist": acc})